...
public class KeyFilesServerApp implements Runnable
{
	...
	public static String[] usernames = {"A", "B", "C"}; // hardcoded list of usernames
    ...
    public void start() {
        if(!isStarted) {
            serverThread = new Thread(this, "OtrFTP server thread");
            serverThread.start();
            try {
    ...
    public void run()
    {
        while(!hasBeenStopped.get())
        {
            ...
            try {
                ...
                new Thread(new KeyFilesConnection(socket), "Connection server thread").start();
